/* CF-ADAPTER (migrated): the escalation itself is recorded first; S.escalations then carries the same id */
function logRung(i){
  const l=LADDER[i],note=document.getElementById('rg_note')?.value.trim();
  let esc;try{esc=CF.recordEscalation(i,l.t,note,{pre:l.pre,target:l.target,sought:l.sought,next:l.next});}catch(err){cfBlocked();return;}
  S.escalations.push({id:esc.id,rung:i,title:l.t,pre:l.pre,target:l.target,sought:l.sought,next:l.next,note,ts:esc.ts});
  CF.markValidated(S,['escalations']);
  logEvent('escalation','Reinforcement used — Rung '+(i+1)+': '+l.t+(note?' — '+note:''),undefined,{validated:true});
...
/* ---------------- chain ---------------- */
/* CF-ADAPTER (migrated): each chain-cell edit is its own recorded fact, sourced to the live interaction if there is one */
function chainSetField(i,q,val){
  try{CF.recordChainAnswer(i,q,val,S.live?S.live.id:'');}catch(err){cfBlocked();return;}
  S.chain[i].fields[q]=val;CF.markValidated(S,['chain']);save();renderChain();
}
function renderChain(){
  document.getElementById('chainBody').innerHTML=S.chain.map((n,i)=>{
    ...
    return `<div class="chainnode ${i===0?'open':''}"><div class="hd" onclick="this.parentElement.classList.toggle('open')">
      ...
      <div class="chainfields">${SIX_Q.map(q=>`<input class="${n.fields[q]?'':'empty'}" placeholder="${q}" value="${esc(n.fields[q]||'')}" onchange="chainSetField(${i},'${q}',this.value)">`).join('')}</div></div>`;
